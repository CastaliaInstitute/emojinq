...
def main() -> None:
    ...
    required = (
        ...
        "Color wash · standard + 71 PUA · ink fallback",
        'fetch(`${puaColorRoot}manifest.json`)',
        "for (const source of puaColorScope.sources) COLOR_PUA_SOURCES.add(source);",
        '<option value="blind">Label-blind test</option>',
        'name="guess"',
        'name="observer_age_months"',
        'name="choice_free"',
        'review_scale_css_px: reviewMode.value === "blind" ? 32 : null',
        'image.alt = blind ? "" : item.name',
        "Label-blind guess:",
